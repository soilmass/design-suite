...
*Table (V-332) and Data grid (V-333) stay two entries, not one*, despite Vocabulary's "→ distinguish" marker being the same signal that preceded three folds in this document (Tooltip/Popover, Toast/Banner, Progress and spinner). That marker precedes a fold only when the pair also passes A-095's actual test — sharing a parameter set and diverging only in what's present, absent, or fixed — and this pair does not: sorting, filtering, resizing, and per-cell editing are parts a plain table has no version of at all, not a table's own parts left absent or fixed, the same category of difference A-101 Skeleton drew against A-100 Progress and spinner to justify staying separate. Data grid's entry reuses Table's `row[]`, `column[]`, and `header[]` without restating them, the way A-104 Lightbox reuses A-065 Dialog's overlay parameters, and adds only what interaction actually requires.

*Accordion (V-335) and Disclosure (V-336) stay two entries, not one, but for a different reason than Table/Data grid: this was never a fold question.* Vocabulary carries no "→ distinguish" marker between them at all — the seventh slice's note claiming otherwise is corrected above. Disclosure is the atomic unit: a single show-and-hide toggle. Accordion is a group of that unit: "vertically stacked collapsible sections," which is Disclosure repeated and stacked, not a different construction wearing a similar name. The relationship is compositional, the same shape A-081 Facets already established for its own `applied filter[]` (each a chip) — Accordion's entry defines a `section[]` where each section is a Disclosure (A-1XX), plus one accordion-level parameter Disclosure alone has no use for: whether more than one section may be expanded at once, or opening one closes the rest.

*Infinite scroll (V-354)* shares no parameter set with A-080 Pagination's `page control[]`, `current indicator`, and `overflow marker` — it is an alternative mechanism for the same underlying need (reaching more of a long result set), not a variant of pagination's own construction, so it is not folded with Pagination and does not extend it the way Data grid extends Table. Its own entry reuses A-100 Progress and spinner's construction for its loading indicator rather than restating it, and cites A-080 Pagination as the alternative mechanism a design chooses instead of, the same way A-104 Lightbox cites A-065 Dialog for parameters it reuses without owning them itself.
